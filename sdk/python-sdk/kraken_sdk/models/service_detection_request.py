...
class ServiceDetectionRequest(BaseModel):
    ...
    address: StrictStr = Field(description="The ip address the service listens on")
    port: Annotated[int, Field(strict=True, ge=0)] = Field(description="The port the service listens on")
    timeout: Annotated[int, Field(strict=True, ge=0)] = Field(description="Time to wait for a response after sending the payload (or after establishing a connection, if not payload is to be sent)  The timeout is specified in milliseconds.")
    workspace_uuid: StrictStr = Field(description="The workspace to execute the attack in")
    __properties: ClassVar[List[str]] = ["leech_uuid", "address", "port", "timeout", "workspace_uuid"]
