...
pub static KRAKEN_URI: EnvVar<Url> = EnvVar::required("KRAKEN_URI");

/// The fake domain the kraken's cert is valid for
pub static KRAKEN_SNI: EnvVar<String> = EnvVar::required("KRAKEN_SNI");

/// PEM encoded CA managed by kraken
pub static KRAKEN_CA: EnvVar<String> = EnvVar::required("KRAKEN_CA");

/// PEM encoded certificate to present when communicating with kraken over grpc
pub static LEECH_CERT: EnvVar<String> = EnvVar::required("LEECH_CERT");

/// PEM encoded private key for the `leech_key`
pub static LEECH_KEY: EnvVar<String> = EnvVar::required("LEECH_KEY");

/// Random string the leech has to present when connecting to kraken during the backlog process.
pub static LEECH_SECRET: EnvVar<String> = EnvVar::required("LEECH_SECRET");

/// Bundle of all kraken variables combined
pub static KRAKEN: LazyLock<KrakenConfig> = LazyLock::new(|| KrakenConfig {
    kraken_uri: KRAKEN_URI.clone(),
    kraken_sni: KRAKEN_SNI.clone(),
    kraken_ca: KRAKEN_CA.clone(),
});
