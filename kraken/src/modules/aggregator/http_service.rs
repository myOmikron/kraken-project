...
async fn aggregate(data: HttpServiceAggregationData) -> Result<Uuid, rorm::Error> {
    ...
    // TODO: decide which fields contribute to identification and which can be updated
    let existing = query!(&mut tx, (HttpService::F.uuid,))
        .condition(and![
    ...
    let http_service_uuid = if let Some((uuid,)) = existing {
        uuid
    } else {
